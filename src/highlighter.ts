...
export async function getHighlighter(options: HighlighterOptions): Promise<Highlighter> {
  ...
  function codeToHtml(code: string, lang?: string, theme?: string): string
  function codeToHtml(code: string, arg1: string | HtmlOptions = 'text', arg2?: string) {
  ...
  async function loadTheme(theme: IShikiTheme | string) {
    await _registry.loadTheme(theme)
  }

  async function loadLanguage(lang: ILanguageRegistration) {
    _resolver.addLanguage(lang)
    await _registry.loadLanguage(lang)
  }
